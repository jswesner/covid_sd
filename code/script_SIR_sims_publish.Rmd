...
knitr::opts_chunk$set(echo = F,
                      results = "hide", message=FALSE, warning=FALSE, paged.print=FALSE)
library(tidyverse)
library(here)
library(janitor)
library(viridis)
...
ggsave(plot_r, file = "plot_r.jpg", width = 6, height = 6)
...
both
ggsave(both, file = here::here("plots/jeff_cumulative_cases.jpg"), width = 8, height = 8)
```
...
both_hosp
ggsave(both_hosp, file = here::here("plots/jeff_hosp.jpg"), width = 8, height = 8)
```
...
The model with a 7-day generation time appears to best match the actual hospitalization data. It indicates that peak resource use will occur ~June 1 with the following needs: ~5900 hospital beds (95% CI: 4900 to 6900), ~2400 ICU beds (1990 to 2800), and ~1700 Ventilators (1400 to 1900).
```{r echo=FALSE, message=FALSE, warning=FALSE}
beds_needed <- all_hosp %>% 
  filter(gt == 7) %>% 
  group_by(date, medical_need, gt) %>% 
  mutate(number = value*884235) %>% 
  drop_na(number) %>% 
  group_by(R0, medical_need, gt) %>% 
  mutate(max_beds = max(number)) %>% 
  distinct(max_beds) %>% 
  group_by(medical_need, gt) %>% 
  summarize(mean_max = round(mean(max_beds),0),
            lci_max = round(quantile(max_beds, probs = 0.025),0),
            uci_max = round(quantile(max_beds, probs = 0.975),0)) %>% 
  select(-gt) %>% 
  ungroup() %>% 
